...
/* $('#DirectorDetails').off().on('keydown','input[type=text], input[type=number]',(event) => {
        if(event.which === 13){ // Detect enter keypress
           // args.jsGrid.updateItem(); // Update the row
			$("#DirectorDetails").jsGrid("updateItem");
        }
    });
 */
...
ButtonField.prototype = new jsGrid.Field({
                ...
                itemTemplate: function (value, item) {
                    ...
                    var $customButton = $("<button>")
                            ...
                            .click(function (e) {
								...
								if(item.Voting_Required === "C" || item.Voting_Required === "S") { HaveDirector=true; $("#DirectorDetails").jsGrid("loadData");$('#DirectorDetails').show();} 
								//$('#AgendaModal').width('99%');
	...
	onItemEditing: function (args) { PreviousVotingType = args.item.Voting_Required ; },
	...
	onItemUpdated: function (args) { $("#AgendaGrid").jsGrid("loadData"); },
...
if(args.item.Voting_Required === "C" || args.item.Voting_Required === "S") {
	...
	onError: function(args) {
     ...
     fields: [
	  ...
	  { name: "BTN",title: "", type: "button",editing: false, width: 10 },
      { name: "Sort_ID", title: "Sort", type: "text",editing: true, width: 30,validate: "required" },
      { name: "AGENDA_ID",title: "AG ID",type: "text",width: 25, validate: "required"},
	  {	name: "Special_Formula",title: "Formula",type: "select",width: 100,
	  items: [     
	...
	editing: true,
	items: [
     { Name: "Majorty", Id: 'Majorty' },
     { Name: "One Third", Id: 'OneThird' },
	 { Name: "Two Third", Id: 'TwoThird' },
	 { Name: "One Half", Id: 'Half' }
    ], 		
    validate: "required"
      },
	  {
    ...
    type: "text", 
    width: 30,
    validate: { validator: "range", param: [0,50] }
      },
	  {	
    name: "Voting_Started",
